...
class ChatRepositoriesImpl extends BaseApi implements ChatRepositories {
  ...
  Future<SResult<String>> sendMessage(List<Chat> chats) async =>
      await apiCall<dynamic, String>(
          mapper: (data) => data["choices"][0]?["message"]["content"] ?? "",
          request: () async => await _gptApi.chat(body: {
                "model": GptConstant.model,
                "messages":
                    chats.map((e) => ChatModel.fromEntity(e).toJson).toList(),
